...
test('real journal reads honor book and inclusive range', async () => {
  ...
  expect(report.trialBalance.totals).toEqual({ debit: 350, credit: 350, difference: 0 });
...
test('branch report ports use authoritative report numbers', async () => {
  ...
  const ports = createAccountingReportPorts(db, guard);
  expect((await ports.profitAndLoss({ from: '2026-02-01', to: '2026-02-28' }, { kind: 'company' }, scope)).revenue).toBe(250);
  expect((await ports.trialBalance('2026-02-28', { kind: 'company' }, scope)).totals.debit).toBe(350);
  expect((await ports.balanceSheet('2026-02-28', { kind: 'company' }, scope)).assets).toBe(350);
});
test('balance sheet refuses unlabelled provisional location figures', async () => {
  const { db, guard } = await setup();
  const ports = createAccountingReportPorts(db, guard);
  await expect(ports.balanceSheet(scope.today, { kind: 'locations', ids: ['shop'] }, scope)).rejects.toThrow('PROVISIONAL_LOCATION_COGS');
});
