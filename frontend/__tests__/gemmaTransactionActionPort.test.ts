...
test('confirmation posts once through the transaction-bound V2 service', async () => {
  ...
  try {
    ...
    const confirm = createProposalExecutor(store, {
      currentScope: async () => scope,
      entityRevisions: async () => ({}),
      isPeriodOpen: async () => true,
      canApply: async operation => operation !== 'create_quote',
      apply: createTransactionActionPort(),
    });
    const first = await confirm(proposal.id);
    expect(first).toMatchObject({ kind: 'applied', replayed: false, result: { message: 'Expense recorded ✓' } });
    expect((first as any).result.committedIds).toHaveLength(2);
    expect(await db.first("SELECT COUNT(*) count FROM v2_sources WHERE book_id='a' AND type='expense'")).toEqual({ count: 1 });
    const replay = await confirm(proposal.id);
    expect(replay).toMatchObject({ kind: 'applied', replayed: true });
    expect(await db.first("SELECT COUNT(*) count FROM v2_sources WHERE book_id='a' AND type='expense'")).toEqual({ count: 1 });
  } finally { close(); }
});

test('every advertised live proposal has a real transaction-bound domain route', async () => {
  const { runner: db, close } = makeNodeRunner();
  try {
    await initSchema(db);
    await initializeV2Book(db, {
      ...
      personas: ['custom'],
      members: [{ name: 'Partner A', openingContribution: 0, profitSharePct: 100 }],
    });
    await db.run("INSERT INTO meta(key,value) VALUES('v2_active_book_id','a') ON CONFLICT(key) DO UPDATE SET value='a'");
    ...
    const params: Record<string, Record<string, unknown>> = {
      ...
      create_receipt: { amount: 18, mode: 'cash_sale', method: 'cash' },
      add_capital: { amount: 19, partnerName: 'Partner A' },
      create_drawing: { amount: 20, partnerName: 'Partner A' },
