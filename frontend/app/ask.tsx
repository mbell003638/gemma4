...
export default function AskBooks() {
  ...
  useEffect(() => {
    if (typeof params.assistantDraft !== 'string') return;
    try { const draft = JSON.parse(params.assistantDraft); const text = [draft.action?.replace(/_/g, ' '), draft.amount, draft.currency, draft.counterparty, draft.date, draft.note].filter(Boolean).join(' '); if (text) setInput(text); } catch {}
  ...
  const applyPendingProposal = async () => {
    ...
    try {
      ...
      const result = await executeAssistantProposal(proposal, { confirmed: true }, async () => {
  ...
  const applyPendingDurableProposal = async () => {
    ...
    try {
      ...
      setPendingDurableProposal(null);
      setMessages((m) => [...m, { role: "assistant", text: outcome.replayed ? "That change was already recorded." : "Ledgr change recorded ✓" }]);
    } catch (err: any) {
      if (!await isCurrent(true)) return;
      setMessages((m) => [...m, { role: "assistant", text: `I couldn't apply that change: ${err?.message || "error"}` }]);
    } finally {
  ...
  const cancelPendingDurableProposal = async (includeUserMessage = false) => {
    ...
    setMessages((m) => [...m, ...(includeUserMessage ? [{ role: "user" as const, text: "Cancel" }] : []), { role: "assistant", text: "Okay — I did not change your books." }]);
  };
  ...
  const send = async (text: string) => {
    ...
    if (pendingDurableProposal || pendingProposal) {
      await handlePendingConfirmation(q, {
        ...
        clarify: async () => {
          if (!await isCurrent()) return;
          setMessages((m) => [...m, { role: "assistant", text: "Nothing was applied. Cancel this proposal before revising it, or use Apply to record exactly the displayed change." }]);
        },
      });
      return;
    }
        ...
        <View style={[styles.inputBar, { paddingBottom: composerBottomPad }]}>
          <View style={styles.inputWrapper}>
            ...
            {input.trim().length > 0 ? (
              ...
              <Pressable accessibilityRole="button" accessibilityLabel="Open voice transaction assistant" style={styles.micBtn} onPress={() => router.push("/voice")}>
