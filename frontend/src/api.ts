...
export const api = {
  ...
  updateSettings: async (s: any) => {
    const runner = activeSqlRunner();
    let context: Awaited<ReturnType<V2AppService['activeContext']>> = null;
    let sharedConfig: V2BookConfigUpdate | null = null;
    if (runner) {
      const service = new V2AppService(runner);
      context = await service.activeContext();
      if (context) {
        ...
        const applyShared = async () => {
          if (sharedConfig) await new V2BookConfigRepository(runner).updateBookConfig(context!.bookId, sharedConfig);
          if (Array.isArray(s.enabledFeatures)) await writeV2BookPrefs(runner, context!.bookId, { enabledFeatures: s.enabledFeatures });
        };
        if (sharedConfig || Array.isArray(s.enabledFeatures)) {
          await withSyncedMutation(runner, {
            commandType: 'book.config.patch', aggregateType: 'book', aggregateId: context.bookId,
            payload: { ...(sharedConfig ? { config: sharedConfig } : {}), ...(Array.isArray(s.enabledFeatures) ? { enabledFeatures: s.enabledFeatures } : {}) },
          }, applyShared);
        }
  ...
  analyzeDocument: async (input: { base64?: string; mimeType?: string; text?: string; uri?: string }) => {
    const [config, settings, suppliers, customers, capitalAccounts] = await Promise.all([
      getAIConfig(), db.getSettings(), api.listSuppliers(), api.listDebtors(), api.listInvestors(),
    ]);
    const route = await analyzeDocumentLocalFirst({
      ...
      analyzeCloud: (cloudInput) => {
        if (cloudInput.base64 && cloudInput.base64.length > MAX_AI_DOCUMENT_BASE64_CHARS) {
          throw new Error('That document is too large for cloud analysis. Use a file under 6MB, a lower-resolution image, or paste the text.');
        }
